...
const CartHeader = () => {
	// управление кнопкой корзины в хеддере
	const { items, totalPrice } = useSelector(selectorCart);
	const headerTotalCount = items.reduce((final: any, item: any) => final + item.count, 0);

	return (
		<Link to="/cart" id="cart" className="header-control">
			<span className="header-control__title">
				cart ({headerTotalCount} | ${totalPrice})
			</span>
