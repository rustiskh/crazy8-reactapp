...
const FilterSize: React.FC = () => {
	...
	// React.useEffect(() => {
	// 	axios("http://localhost:3005/products").then((response) => {
	// 		// const newArr = response.data.map((e) => console.log(e.sizes));
	// 		const newArr = response.data.reduce((final, cur) => {
	// 			return [...final, ...cur.sizes];
	// 		}, []);
		...
		<div id="filter-size" data-filter-type="checkbox" className="catalog-filter catalog-filter_opened">
			...
			<div className="catalog-filter__body">
				<ul className="checkbox-filter">
					{sizes.map((filterItem, i) => (
						<li key={i} className="checkbox-filter__item">
